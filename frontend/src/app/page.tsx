...
export default function LegacyOSDashboard() {
  ...
  return (
    <div className="flex h-screen bg-gradient-to-br from-[#041E24] via-[#0A2E2F] to-[#041517] text-neutral-200 font-sans overflow-hidden">
      ...
      <aside className="w-64 border-r border-white/10 bg-white/5 backdrop-blur-2xl flex flex-col justify-between hidden md:flex z-50 shadow-xl shadow-black/20">
        <div>
          ...
          <nav className="p-4 space-y-2 mt-4">
            <button onClick={() => setActiveMenu('board')} className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium transition-all ${activeMenu === 'board' ? 'bg-blue-500/20 text-blue-300 shadow-sm border border-blue-500/20' : 'text-neutral-400 hover:bg-white/5 hover:text-white'}`}>
              <Users className="w-4 h-4" /> Shadow Board
            </button>
            <button onClick={() => setActiveMenu('simulation')} className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium transition-all ${activeMenu === 'simulation' ? 'bg-blue-500/20 text-blue-300 shadow-sm border border-blue-500/20' : 'text-neutral-400 hover:bg-white/5 hover:text-white'}`}>
              <Eye className="w-4 h-4" /> Digital Twins
            </button>
            <button onClick={() => setActiveMenu('memory')} className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium transition-all ${activeMenu === 'memory' ? 'bg-blue-500/20 text-blue-300 shadow-sm border border-blue-500/20' : 'text-neutral-400 hover:bg-white/5 hover:text-white'}`}>
      ...
      <main className="flex-1 flex flex-col h-screen overflow-hidden">
        ...
        <div className="flex-1 p-8 flex gap-8 overflow-hidden">
          ...
          <div className="w-1/3 flex flex-col gap-4 h-full min-w-[320px]">
            ...
            <div className="bg-white/5 backdrop-blur-xl rounded-2xl shadow-xl shadow-black/20 border border-white/10 flex-1 flex flex-col min-h-0 overflow-hidden relative">
              <div className="px-5 py-3 border-b border-white/10 bg-white/5 flex items-center gap-2">
                ...
                <h3 className="text-xs font-bold uppercase tracking-wider text-blue-200/80">Execution Log</h3>
              </div>
              ...
              <div className="flex-1 overflow-y-auto p-5 space-y-4 font-mono text-sm">
                ...
                <div className="mb-6 p-4 rounded-xl bg-blue-500/10 border border-blue-500/20 backdrop-blur-sm">
                  <h4 className="text-blue-300 font-bold text-xs uppercase mb-1 drop-shadow-sm">
            ...
            {/* Input Box */}
            <div className="bg-white/5 backdrop-blur-xl p-2 pl-5 rounded-2xl shadow-xl shadow-black/20 border border-white/10 flex items-center gap-3 shrink-0 focus-within:border-blue-400/50 transition-colors">
              <input 
                type="text" 
                value={query}
                onChange={(e) => setQuery(e.target.value)}
                placeholder="Deploy a strategy or pitch..."
                className="flex-1 bg-transparent outline-none text-sm text-white placeholder-blue-200/50"
                onKeyDown={(e) => e.key === 'Enter' && startSimulation()}
              />
              <button 
                onClick={startSimulation}
                disabled={isSimulating}
                className="bg-blue-500 text-white p-3 rounded-xl hover:bg-blue-400 shadow-lg shadow-blue-500/20 transition-all disabled:opacity-50"
              >
          ...
          <div className="flex-1 bg-white/5 backdrop-blur-2xl rounded-2xl shadow-xl shadow-black/20 border border-white/10 p-8 flex flex-col relative overflow-hidden h-full">
            <h3 className="text-xs font-bold uppercase tracking-wider text-blue-200/80 mb-6 z-10">
              {activeMenu === 'memory' ? '3D Knowledge Graph' : 'Simulation Canvas'}
            </h3>
            
            {/* Background Aesthetic */}
            <div className="absolute inset-0 flex items-center justify-center opacity-5 pointer-events-none">
               <BrainCircuit className="w-[600px] h-[600px] text-blue-200" />
            </div>
                       ...
                       {logs.filter(l => l.type === 'speak' || l.type === 'reaction' || l.type === 'resolution' || l.type === 'finish').map((log, idx) => (
                         <motion.div 
                           ...
                           className={activeMenu === 'board' ? (
                             `p-5 rounded-xl border backdrop-blur-md ${log.type === 'resolution' || log.type === 'finish' ? 'col-span-full bg-blue-500/20 border-blue-400/40 shadow-[0_0_15px_rgba(96,165,250,0.15)]' : 'bg-white/5 border-white/10'}`
                           ) : (
                         ...
                         >
                           ...
                           <div className="flex-1 min-w-0">
                             <h5 className={`font-bold text-sm mb-1 ${log.type === 'resolution' || log.type === 'finish' ? 'text-blue-300' : 'text-white drop-shadow-sm'}`}>
                               {log.agent || log.persona || 'System Orchestrator'}
                             </h5>
                             <p className="text-xs text-blue-100/70 leading-relaxed truncate whitespace-normal line-clamp-2">{log.message}</p>
                           </div>
                         </motion.div>
                       ))}
                     </div>
                     ...
                     <div className="h-full flex flex-col items-center justify-center text-center max-w-md mx-auto">
                       {isSimulating ? (
                         <motion.div animate={{ scale: [1, 1.05, 1] }} transition={{ repeat: Infinity, duration: 2 }}>
                           <Users className="w-16 h-16 mx-auto mb-6 text-blue-400 opacity-80" />
                           <h4 className="text-lg font-medium text-white mb-2 drop-shadow-sm">Swarm Active</h4>
                           <p className="text-sm text-blue-200/60">Agents are currently analyzing parameters and debating outcomes in real-time.</p>
                         </motion.div>
                       ) : (
                         <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                           ...
                           <p className="text-sm text-blue-200/40">Submit a query to initialize the AI multi-agent swarm.</p>
                         </motion.div>
                       )}
                     </div>
                   )}
